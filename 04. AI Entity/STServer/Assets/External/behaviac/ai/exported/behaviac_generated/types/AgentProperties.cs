...
namespace behaviac
{
	...
	public class BehaviorLoaderImplement : BehaviorLoader
	{
		...
		{
			AgentMeta.TotalSignature = 3703261387;
			...
			// STCommonAI
			meta = new AgentMeta(3448658374);
			AgentMeta._AgentMetas_[95024505] = meta;
			meta.RegisterMethod(1045109914, new CAgentStaticMethodVoid<string>(delegate(string param0) { STCommonAI.LogMessage(param0); }));
			meta.RegisterMethod(2341147270, new CAgentMethodVoid(delegate(Agent self) { ((STCommonAI)self).MoveBack(); }));
			meta.RegisterMethod(3654410308, new CAgentMethodVoid(delegate(Agent self) { ((STCommonAI)self).MoveForward(); }));
			meta.RegisterMethod(1883830213, new CAgentMethodVoid(delegate(Agent self) { ((STCommonAI)self).MoveLeft(); }));
			meta.RegisterMethod(1082409741, new CAgentMethodVoid(delegate(Agent self) { ((STCommonAI)self).MoveRight(); }));
			meta.RegisterMethod(2521019022, new CMethod_behaviac_Agent_VectorAdd());
			meta.RegisterMethod(2306090221, new CMethod_behaviac_Agent_VectorClear());
			meta.RegisterMethod(3483755530, new CMethod_behaviac_Agent_VectorContains());
			meta.RegisterMethod(505785840, new CMethod_behaviac_Agent_VectorLength());
			...
			AgentMeta.Register<STCommonAI>("STCommonAI");
			return true;
		}
		...
		public override bool UnLoad()
		{
			...
			AgentMeta.UnRegister<STCommonAI>("STCommonAI");
			return true;
		}
	}
}
